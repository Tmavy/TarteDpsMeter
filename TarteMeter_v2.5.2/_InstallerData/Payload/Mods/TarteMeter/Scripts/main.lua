local MOD_NAME = "TarteMeter v2.5.2"
...
local MONSTER_POSITION_CACHE_SECONDS = 0.080
local TARGET_MATCH_MAX_DISTANCE = 3000.0
local ATTRIBUTION_DIAGNOSTIC_LIMIT = 12
...
local monster_cache = {}
local monster_position_cache = {}
local monster_position_cache_time = -999.0
...
local function refresh_monster_cache()
    ...
    monster_position_cache_time = -999.0
    return #monster_cache > 0
end

local function get_valid_monster_cache()
    local previous_count = #monster_cache
    local valid = {}
    ...
    monster_cache = valid
    if #monster_cache ~= previous_count then
        monster_position_cache = {}
        monster_position_cache_time = -999.0
    end
...
local function rebuild_monster_position_cache(force)
    local t = now()
    if not force
        and #monster_position_cache > 0
        and (t - monster_position_cache_time) <= MONSTER_POSITION_CACHE_SECONDS
    then
        return monster_position_cache
    end

    local positions = {}
    for _, monster in ipairs(get_valid_monster_cache()) do
    ...
    monster_position_cache = positions
    monster_position_cache_time = t
    return monster_position_cache
end

local function nearest_cached_monster(hit_location, force_positions)
    local hx, hy, hz = vector_xyz(hit_location)
    local monsters = get_valid_monster_cache()

    if #monsters == 0 then
        return nil, nil, "monster_cache_empty"
    end

    -- A single live monster is unambiguous even if the game supplied no vector.
    if hx == nil and #monsters == 1 then
        return monsters[1], nil, "single_monster"
    end
    if hx == nil then
    ...
    for _, entry in ipairs(rebuild_monster_position_cache(force_positions)) do
        local dx = entry.x - hx
        local dy = entry.y - hy
        local dz = entry.z - hz
    ...
    return nearest, distance, "hit_location_nearest"
end

local function find_damage_target(hit_location)
    local target, distance, method = nearest_cached_monster(
        hit_location,
        false
    )
    if target ~= nil then return target, distance, method end

    -- New waves/adds can spawn while an older cached monster still exists.
    -- Refresh the world list only after an actual target-location miss.
    if refresh_monster_cache() then
        target, distance, method = nearest_cached_monster(
            hit_location,
            true
        )
        if target ~= nil then
            return target, distance, "refreshed_" .. tostring(method)
        end
    end

    return nil, distance, method
end
...
local function update_active_character()
    ...
    if changed then
        ...
        swap_time = now()
        carryover_enabled = SWAP_CARRY_SECONDS > 0.0
            and old_name ~= nil
            and old_name ~= "Unknown"
...
local function process_pending_damage()
    ...
    for _, event in ipairs(pending_damage) do
        if (t - event.time) < DAMAGE_DEFER_SECONDS then
            ...
            -- LastAttacker is reliable only on the monster that actually received
            -- this damage number. The old aggregate scan mixed stale LastAttacker
            -- values from unrelated monsters and could assign Theresa's lingering
            -- damage to the currently active Tarte.
            local target_owner = event.target_owner
            local target_method = event.target_method

            if (target_owner == nil or target_owner == "Unknown")
                and event.target ~= nil
            then
                target_owner, target_method = resolve_target_last_attacker(
                    event.target,
                    "deferred"
                )
            end
            ...
            elseif source ~= nil then
                ...
                if attribution_diagnostic_count < ATTRIBUTION_DIAGNOSTIC_LIMIT then
                    ...
                    append_line("ATTRIBUTION_ACTIVE_FALLBACK", string.format(
                        "damage=%.1f target=%s target_match=%s captured_active=%d:%s",
                        event.damage,
                        event.target_key or "nil",
                        tostring(event.target_match_method or "unknown"),
                        event.active_index,
                        event.active_name
                    ))
                end
...
install_hook(
    ...
    function(Context, Damage, HitLocation, IsPlayer, bCritical)
        ...
        append_line("DAMAGE_PENDING", string.format(
            "damage=%.1f critical=%s active=%d:%s target=%s distance=%s target_owner=%s pending=%d",
            damage,
            tostring(critical),
            active_pc,
            active_name,
...
append_line("INIT_DONE", string.format(
    "dynamic_pawn=true cached_controller=true target_last_attacker=true hit_location_targeting=true on_demand_monster_cache=true owner_cache_pruning=true stable_map_travel=true roster_refresh=%.1fs state_write=%.1fs combat_end_idle=%.1fs auto_save_min=%.1fs source_keep=%.1fs deferred=%.0fms",
