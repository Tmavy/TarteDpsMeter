...
function Test-GameExe {
    ...
    return (
        ...
        [IO.Path]::GetFileName($Path) -ieq 'DSClient-Win64-Shipping.exe'
    )
}
...
$installInfo.Text = "Installs or updates TarteMeter. UE4SS is added only when required.`r`nBattle history, settings, hotkeys, and unrelated mods are preserved."
...
$launchAfterInstallCheck = New-Object Windows.Forms.CheckBox
$launchAfterInstallCheck.Text = 'Launch TarteMeter after installation'
$launchAfterInstallCheck.Checked = $true
$launchAfterInstallCheck.Location = New-Object Drawing.Point(17, 148)
$launchAfterInstallCheck.Size = New-Object Drawing.Size(324, 22)
$launchAfterInstallCheck.ForeColor = [Drawing.Color]::FromArgb(218, 226, 235)
$launchAfterInstallCheck.BackColor = $installPanel.BackColor
$installPanel.Controls.Add($launchAfterInstallCheck)

$installButton = New-Object Windows.Forms.Button
$installButton.Text = 'Install or Update TarteMeter'
$installButton.Location = New-Object Drawing.Point(16, 174)
...
function Set-ActionsEnabled {
    ...
    $launchAfterInstallCheck.Enabled = $Enabled
...
$launchButton.Add_Click({
    if (-not (Test-GameExe $pathBox.Text)) {
        [void][Windows.Forms.MessageBox]::Show(
    ...
    try {
        ...
        $ready = Start-TarteMeterOverlay -GameExe $pathBox.Text -WaitForReady $true
        if ($ready) {
...
$installButton.Add_Click({
    ...
    try {
        Install-TarteMeter `
            -GameExe $pathBox.Text `
            -RepairUE4SS $repairCheck.Checked

        $overlayReady = $false
        if ($launchAfterInstallCheck.Checked) {
            Set-Status 'Launching TarteMeter and checking the window...' ([Drawing.Color]::FromArgb(235, 201, 129))
            $overlayReady = Start-TarteMeterOverlay -GameExe $pathBox.Text -WaitForReady $true
        }

        if ($launchAfterInstallCheck.Checked -and $overlayReady) {
            Set-Status 'Setup complete. TarteMeter is running.' ([Drawing.Color]::FromArgb(111, 201, 160))
            $message = "TarteMeter was installed successfully and the overlay window opened.`r`n`r`nStart or restart DragonSword through Steam."
        }
        elseif ($launchAfterInstallCheck.Checked) {
            Set-Status 'Installed, but the TarteMeter window did not open.' ([Drawing.Color]::FromArgb(224, 172, 112))
            $message = "TarteMeter was installed, but its window did not open within 20 seconds.`r`n`r`nA detailed report was created in Mods\TarteMeter\startup_error.txt."
        }
        else {
            Set-Status 'Setup completed successfully.' ([Drawing.Color]::FromArgb(111, 201, 160))
            $message = "TarteMeter was installed successfully.`r`n`r`nStart DragonSword normally through Steam."
        }
...
$removeButton.Add_Click({
    ...
    try {
        Uninstall-TarteMeter `
            -GameExe $pathBox.Text `
            -RemoveUE4SS $removeUE4SSCheck.Checked
