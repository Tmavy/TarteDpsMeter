...
$settingsPath = Join-Path $PSScriptRoot "window_settings_v11_compact.ini"
...
Write-StartupDiagnostic "Startup architecture" "detached PowerShell; overlay mutex v301; build 3.0.1; UI COMPACT"

$createdNew = $false
$instanceMutex = New-Object System.Threading.Mutex(
    $true,
    "Local\TarteMeterOverlay_v301",
    [ref]$createdNew
)
if (-not $createdNew) {
    try {
  ...
  <Grid>
   ...
   <Grid x:Name="TitleBar" Grid.Row="0" Background="{DynamicResource ThemeChromeBrush}">
    ...
    <StackPanel Grid.Column="1" Orientation="Horizontal" VerticalAlignment="Center">
     ...
     <Border x:Name="VersionBadge" Background="{DynamicResource ThemeBadgeBrush}" CornerRadius="4" Padding="5,1" Margin="7,0,0,0"><TextBlock Text="v3.0.1" Foreground="{DynamicResource ThemeAccentTextBrush}" FontSize="{DynamicResource MeterTextSmall}"/></Border>
...
function Process-OverlayShowRequest {
    ...
    try {
        ...
        [IO.File]::WriteAllText(
            $readyPath,
            ("TarteMeter v3.0.1 ready | " + [DateTime]::Now.ToString("o")),
            [Text.Encoding]::UTF8
        )
...
try {
    if (Test-Path -LiteralPath $runtimeVersionPath) {
        ...
        if ($runtimeVersion -ne "TarteMeter v3.0.1") {
            Write-StartupDiagnostic "Runtime version mismatch" ("expected=TarteMeter v3.0.1 actual=" + $runtimeVersion)
        }
        else {
...
$applyThemeChange = {
    ...
    try {
        ...
        if (-not (Write-WindowSettings)) {
            throw (
                "The theme was applied, but window_settings_v11_compact.ini " +
                "could not be saved."
            )
        }
...
$window.Add_Loaded({
    ...
    try {
        [IO.File]::WriteAllText(
            $windowBuildPath,
            "TarteMeter v3.0.1 | mutex v301 | UI COMPACT",
            [Text.Encoding]::ASCII
        )
        [void](Write-WindowSettings)
    }
