...
* Added a dedicated gear settings panel for opacity, click-through, always-on-top, and text size.
* Renamed HISTORY to LOGS, removed the raw log tab, and removed SAVE/F7.
* RESET now archives before clearing and retains unsaved data if the history write fails.
* Added stable character-specific colors for damage bars.
* Removed duplicate F6 handling, recurring monster enumeration, and unbounded owner-cache growth.
* Fixed automatic-save duration/DPS and Lua declaration ordering.
* Optimized state reads, long histories, event grids, and timeline rendering.
* Preserved conservative damage attribution with SWAP_CARRY_SECONDS = 0.
* Fixed installer upgrades so existing logs, settings, mods.txt entries, and original-file state are preserved.
